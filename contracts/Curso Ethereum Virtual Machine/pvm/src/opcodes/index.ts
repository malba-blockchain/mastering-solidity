...
const Opcodes: {
  ...
  0x55: new Instruction(
    ...
    async (ctx: ExecutionContext) => {
      ...
      const currentValue = storedValue ? bufferToBigInt(storedValue): BigInt(0);


      const originalStoredValue = await ctx.originalStorage.get(
        setLengthLeft(bigIntToBuffer(key), 32)
      );
      const originalValue = originalStoredValue ? bufferToBigInt(originalStoredValue): BigInt(0);
